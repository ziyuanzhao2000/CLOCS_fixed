...
dataset_list = ['physionet','physionet2017','cardiology','ptb','fetal','physionet2016','physionet2020','chapman',
                'chapman_pvc','emg', 'sleepEDF','epilepsy','pFD_A','pFD_B']#,'cipa']
batch_size_list = [256, 256, 16, 64, 64, 256, 256, 256, 256, 4, 256, 4, 256, 4]#, 512]
lr_list = [1e-4, 1e-4, 1e-4, 5e-5, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4]#, 1e-4]
nleads = 12 # 12 | 4
if nleads == 12:
    leads_list = [None,None,None,'i','Abdomen 1','i',"['I', 'II', 'III', 'aVL', 'aVR', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']","['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']","['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']",
                  "['I']","['I']","['I']", "['I']", "['I']"] #'II' for one lead, ['II','V1',etc.] for more leads
elif nleads == 4:
...
trial_to_run_list =  ['Fine_Tuning'] #['Linear','Linear','Linear','Linear'] #['Fine-Tuning','Fine-Tuning','Fine-Tuning','Fine-Tuning'] #['Linear','Linear','Linear','Linear'] #['Fine-Tuning','Fine-Tuning','Fine-Tuning','Fine-Tuning'] #['Fine-Tuning','Fine-Tuning','Fine-Tuning','Fine-Tuning']  #['Linear','Linear','Linear','Linear']  #['Random']#,'Fine-Tuning','Fine-Tuning','Fine-Tuning']#['SimCLR','CMSC','CMLC','CMSMLC'] #current trial to run and perform training # Fine-Tuning | Same as trial_to_load
embedding_dim_list = [320] #320,256,128,64,32
downstream_dataset_list = ['pFD_A']#,'physionet2020'] #dataset for pretraininng # 'chapman' | 'physionet2020'
