...
#%%
dataset = 'chapman'
basepath = f'{os.getcwd()}/data/chapman_ecg'
...
#%%
print("before removal", inputs_dict['ecg'][1]['train']['All Terms'].shape)
...
for modality in modality_list:
    for fraction in fraction_list:
        for phase in phases:
            for term in terms:
                inputs_dict[modality][fraction][phase][term] = inputs_dict[modality][fraction][phase][term].reshape((-1, inputs_dict[modality][fraction][phase][term].shape[-1]))
                outputs_dict[modality][fraction][phase][term] = outputs_dict[modality][fraction][phase][term].reshape((-1))
                pids[modality][fraction][phase][term] = pids[modality][fraction][phase][term].reshape((-1))
